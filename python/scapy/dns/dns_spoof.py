#!/usr/bin/env python3
# DNS spoofer and forwarder using scapy
from scapy.all import *

IFACE = "lo" # or default interface
...
def dns_responder(local_ip: str):
    ...
    def forward_dns(orig_pkt: IP):
        ...
        resp_pkt = IP(dat=orig_pkt[IP].src, src=DNS_SERVER_IP)/UDP(dport=orig_pkt[UDP].sport)/DNS()
    ...
    def get_response(pkt: IP):
        if (
            ...
            if "trailers.apple.com" in str(pkt["DNS Question Record"].qname):
                spf_resp = IP(dst=pkt[IP].src)/UDP(dport=pkt[UDP].sport, sport=53)/DNS(id=pkt[DNS].id, ancount=1, an=DNSRR(rrname=pkt[DNSQR].qname, rdata=local_ip)/
